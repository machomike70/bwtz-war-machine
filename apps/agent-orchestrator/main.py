...
def orchestrate(request: OrchestrateRequest):
    ...
    if any(word in task_lower for word in ["daily", "batch", "5 posts"]):
        if plan == "Free":
            return {"status": "limited", "message": "Daily batch is Pro only (12 XRP/month).", "powered_by": brand}
        return {"status": "success", "message": "Use /daily for the full tailored schedule."}

    if any(word in task_lower for word in ["post", "tweet", "generate post"]):
        return {"status": "success", "message": "Use /daily for the full tailored schedule."}

    elif any(word in task_lower for word in ["reply", "response", "answer"]):
        return {
            "status": "success",
            "suggested_reply": "Love that 🔥 Thanks for pulling up. Let’s go peeps!",
            "powered_by": brand,
            "plan_used": plan
        }

    else:
        return {"status": "success", "message": "Try /daily for the full schedule."}
...
def daily_schedule():
    ...
    return {
        ...
        "tip": "Use these tailored posts for each account. Post 1-2 per day and rotate. Fresh GM/GN framing every time.",
